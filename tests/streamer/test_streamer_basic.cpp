...
public:
    ...
    StreamerTestFixture() {
        ...
        config.l1_layer.buffer_groups = { {"l1", {256 * 1024}, 4} };
...
TEST_CASE_METHOD(StreamerTestFixture, "Streamer Error Handling", "[streamer][error]") {
    ...
    SECTION("Validates L2 bank ID bounds") {
        const size_t invalid_l2_bank_id = config.l2_layer.total_banks() + 1;
    ...
    SECTION("Validates L1 buffer ID bounds") {
        const size_t invalid_l1_id = config.l1_layer.total_buffers() + 1;
