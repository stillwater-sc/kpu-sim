/**
 * @file host_T1_KPU_single_block_matmul.cpp
 * @brief Single 16x16 tile execution model for Host + KPU T1 system
 *
 ...
 *
 * - 4 page buffers (memory controller collation buffers)
 ...
 *   * C[0][0] available at cycle N, C[15][15] at cycle 3*N
 *
 * L1 Stream Buffer Architecture:
 * ----------------------
 * Each L1 stream buffer holds one row/column vector and streams one element per clock
