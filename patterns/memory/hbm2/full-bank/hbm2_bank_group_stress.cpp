// patterns/memory/hbm2/full-bank/hbm2_bank_group_stress.cpp
//
// HBM2 Bank Group Stress Test
// Exercises bank group timing constraints:
// - tRRD_L: ACT to ACT within same bank group (4 cycles)
// - tRRD_S: ACT to ACT across different bank groups (3 cycles)
...
bool test_same_bank_group_stress(HBM2Harness& harness) {
...
bool test_diff_bank_group_stress(HBM2Harness& harness) {
...
bool test_tfaw_constraint(HBM2Harness& harness) {
    ...
    // Expected: at least (16 ACTs / 4 per window) * tFAW = 4 * 16 = 64 cycles just for activates
...
bool test_mixed_bank_groups() {
    ...
    // Test A: All accesses within Bank Group 0
    {
        ...
        HBM2Harness harness_bg0(config);
    ...
    // Test B: Accesses across all 4 bank groups
    {
        auto config = single_pc_config();
        config.queue_depth = 128;
        HBM2Harness harness_all(config);
...
int main() {
    std::cout << "=== HBM2 Bank Group Stress Test ===" << std::endl;
    std::cout << "\nHBM2-2000 Bank Group Timing:" << std::endl;
    ...
    // Test 3: tFAW constraint
    {
        ...
        std::string trace_path = make_trace_path("full-bank", "hbm2_bank_group_stress_trace.json");
