#!/usr/bin/env python3
"""
HBM2 Memory Controller Trace Validator

Validates Chrome Trace JSON files against HBM2 trace invariants.
...
# HBM2-2000 timing parameters (cycles at 1.0 GHz CK)
TIMING = {
    'tRCDRD': 12,   # Row address to column address delay (read)
    'tRCDWR': 6,    # Row address to column address delay (write)
    ...
    'tRRD_S': 3,    # ACT-to-ACT different bank group
    'tCCD_L': 4,    # CAS-to-CAS same bank group
    'tCCD_S': 2,    # CAS-to-CAS different bank group
    'tWTR_L': 8,    # Write-to-read turnaround same BG
    'tWTR_S': 4,    # Write-to-read turnaround diff BG
    'tRTW': 10,     # Read-to-write turnaround
    'tFAW': 16,     # Four activate window
    'tBurst': 2,    # BL4 burst cycles
}

# HBM2 architecture constants
NUM_CHANNELS = 8
NUM_PCS_PER_CHANNEL = 2
BANKS_PER_PC = 16
...
class Event:
    ...
    cycle_issue: int
    cycle_complete: int
    bank: int           # Bank within pseudo-channel (0-15)
    channel: int        # Channel (0-7)
    pc: int             # Pseudo-channel (0-1)
    bank_id: int        # Full encoded bank_id
    ...
    def from_json(cls, item: Dict) -> Optional['Event']:
        ...
        # Decode bank_id: bank_id = channel * 32 + pc * 16 + bank
        bank_id = item.get('tid', 0)
        channel = bank_id // 32
        remainder = bank_id % 32
        pc = remainder // 16
        bank = remainder % 16
...
class TraceValidator:
    """Validates HBM2 memory controller traces against invariants."""
            ...
            if activate and data_op:
                if data_op.cycle_issue < activate.cycle_complete:
                    self.violations.append(Violation(
                        ...
                        pc=activate.pc,
                        fix_hint=f"Data operation must wait for ACTIVATE (tRCD)"
                    ))
    ...
    def _check_inv101_trp_constraint(self):
        ...
        # Group events by (channel, pc, bank)
        bank_events: Dict[tuple, List[Event]] = {}
        for event in self.events:
            key = (event.channel, event.pc, event.bank)
            if key not in bank_events:
                bank_events[key] = []
            bank_events[key].append(event)

        for (channel, pc, bank), events in bank_events.items():
                ...
                if event.name == 'PRECHARGE':
                    ...
                    if gap < 0:
                        self.violations.append(Violation(
                            invariant='INV-101',
                            severity=Severity.ERROR,
                            message=f"Ch{channel} PC{pc} Bank{bank}: ACTIVATE at cycle {event.cycle_issue} before "
    ...
    def _check_inv102_trrd_constraint(self):
        ...
        # Group activates by (channel, pc)
        pc_activates: Dict[tuple, List[Event]] = {}
        for e in self.events:
            if e.name == 'ACTIVATE':
                key = (e.channel, e.pc)
                if key not in pc_activates:
                    pc_activates[key] = []
                pc_activates[key].append(e)

        for (channel, pc), activates in pc_activates.items():
            ...
            for i in range(1, len(activates)):
                ...
                prev_bg = prev.bank // BANKS_PER_BANK_GROUP
                curr_bg = curr.bank // BANKS_PER_BANK_GROUP
                ...
                if gap < required:
                    self.violations.append(Violation(
                        invariant='INV-102',
                        severity=Severity.WARNING,
                        message=f"Ch{channel} PC{pc} tRRD violation: ACT gap={gap} cycles between banks "
            ...
            for i, act in enumerate(activates):
                ...
                if count > 4:
                    self.violations.append(Violation(
                        invariant='INV-103',
                        severity=Severity.WARNING,
                        message=f"Ch{channel} PC{pc} tFAW violation: {count} ACTIVATE commands in "
    ...
    def _check_inv106_tccd_constraint(self):
        ...
        # Group CAS commands by (channel, pc)
        pc_cas: Dict[tuple, List[Event]] = {}
        for e in self.events:
            if 'READ' in e.name or 'WRITE' in e.name:
                key = (e.channel, e.pc)
                if key not in pc_cas:
                    pc_cas[key] = []
                pc_cas[key].append(e)

        for (channel, pc), cas_events in pc_cas.items():
            ...
            for i in range(1, len(cas_events)):
                ...
                if gap < required:
                    self.violations.append(Violation(
                        invariant='INV-106',
                        severity=Severity.WARNING,
                        message=f"Ch{channel} PC{pc} tCCD violation: CAS gap={gap} cycles between banks "
                               f"{prev.bank}->{curr.bank}, required={required} "
                               f"({'same BG' if same_bg else 'diff BG'})",
                        channel=channel,
                        pc=pc,
    ...
    def _check_inv108_trc_constraint(self):
        ...
        # Group activates by (channel, pc, bank)
        bank_activates: Dict[tuple, List[Event]] = {}
        for e in self.events:
            if e.name == 'ACTIVATE':
                key = (e.channel, e.pc, e.bank)
                if key not in bank_activates:
                    bank_activates[key] = []
                bank_activates[key].append(e)

        for (channel, pc, bank), activates in bank_activates.items():
            activates.sort(key=lambda e: e.cycle_issue)
            ...
            for i in range(1, len(activates)):
                ...
                if gap < TIMING['tRC']:
                    self.violations.append(Violation(
                        invariant='INV-108',
                        severity=Severity.ERROR,
                        message=f"Ch{channel} PC{pc} Bank{bank}: tRC violation - gap={gap} cycles between "
    ...
    def print_report(self, json_output: bool = False):
        ...
        print(f"\n{'='*60}")
        print(f"HBM2 Trace Validation Report")
        print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Validate HBM2 memory controller traces against invariants'
    )
