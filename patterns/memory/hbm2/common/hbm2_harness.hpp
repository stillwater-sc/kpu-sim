// patterns/memory/hbm2/common/hbm2_harness.hpp
//
// HBM2-specific test harness for pattern validation
//
...
inline fs::path get_trace_dir(const std::string& category) {
    ...
    fs::path trace_dir = root / "traces" / "memory" / "hbm2" / category;
...
/// HBM2-specific test harness for memory controller patterns
///
/// Provides:
...
class HBM2Harness {
public:
    explicit HBM2Harness(const HBM2MemoryController::Config& config)
        : config_(config)
    ...
    void print_stats(std::ostream& os = std::cout) const {
        ...
        os << "\n=== HBM2 Memory Controller Statistics ===" << std::endl;
    ...
    void print_calibration_data(std::ostream& os = std::cout) const {
        const auto& s = mc_->hbm2_stats();
        os << "\n=== HBM2 Calibration Data ===" << std::endl;
    ...
    /// @param clock_ghz Clock frequency for time conversion (default: 1.0 GHz for HBM2)
    bool export_trace(const std::string& filename, double clock_ghz = 1.0) {
    ...
    bool export_resources(const std::string& filename, double clock_ghz = 1.0) {
        tracker_->finalize(mc_->current_cycle());
