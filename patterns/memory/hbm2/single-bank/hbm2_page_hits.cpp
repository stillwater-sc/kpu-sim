// patterns/memory/hbm2/single-bank/hbm2_page_hits.cpp
//
// HBM2 single bank page hit pattern - sequential reads to same row
//
...
int main() {
    std::cout << "=== HBM2 Single Bank Page Hits Pattern ===" << std::endl;
    std::cout << "Testing sequential reads to same row (page hit scenario)" << std::endl;

    // Use single pseudo-channel config for focused testing
    ...
    // Export trace
    std::string trace_path = make_trace_path("single-bank", "hbm2_page_hits_trace.json");
    harness.export_trace(trace_path);

    std::cout << "\nPASS: HBM2 page hits pattern completed successfully" << std::endl;
    return 0;
}
