// patterns/memory/lpddr5/complex/stream.cpp
//
...
//
// These patterns represent the memory access characteristics of
// the STREAM benchmark, which is the standard for measuring
// sustainable memory bandwidth.
//
// For this simulation, we model the read/write patterns without
// the actual compute (which is memory-bound anyway).
//
...
constexpr int STREAM_SIZE = 256;  // Number of cache lines per array
...
bool test_stream_copy() {
    ...
    // Array layout:
    // Bank 0-3: array a (write destination)
    // Bank 4-7: array b (read source)
    constexpr std::array<uint8_t, 4> write_banks = {0, 1, 2, 3};
    constexpr std::array<uint8_t, 4> read_banks = {4, 5, 6, 7};

    for (int i = 0; i < STREAM_SIZE; ++i) {
        uint32_t row = 100 + (i / 32);
        uint32_t col = (i % 32) * CACHE_LINE_BYTES;
        uint8_t read_bank = read_banks[i % 4];
        uint8_t write_bank = write_banks[i % 4];
    ...
    std::cout << "R:W ratio: 1:1" << std::endl;
...
/// Same pattern as Copy (1 read, 1 write)
bool test_stream_scale() {
    std::cout << "\n=== Test: STREAM Scale ===" << std::endl;
    std::cout << "Pattern: a[i] = q * b[i] (1 read, 1 write per element)" << std::endl;
    ...
    std::cout << "PASS: STREAM Scale completed" << std::endl;
    return true;
}
...
bool test_stream_add() {
    ...
    // Array layout across banks:
    // Bank 0-1: array a (write destination)
    // Bank 2-3: array b (read source 1)
    // Bank 4-5: array c (read source 2)
    // Banks 6-7: unused for this pattern

    for (int i = 0; i < STREAM_SIZE; ++i) {
        uint32_t row = 100 + (i / 32);
        uint32_t col = (i % 32) * CACHE_LINE_BYTES;

        uint8_t bank_a = (i % 2);          // Banks 0-1
        uint8_t bank_b = 2 + (i % 2);      // Banks 2-3
        uint8_t bank_c = 4 + (i % 2);      // Banks 4-5

        // Read b[i] and c[i], then write a[i]
        harness.submit_read(make_address(bank_b, row, col));
        harness.submit_read(make_address(bank_c, row, col));
        harness.submit_write(make_address(bank_a, row, col));
    }
...
/// 2 reads (b, c), 1 write (a) - same access pattern as Add
bool test_stream_triad() {
    ...
    for (int i = 0; i < STREAM_SIZE; ++i) {
        ...
        uint8_t bank_a = (i % 2);
        uint8_t bank_b = 2 + (i % 2);
        uint8_t bank_c = 4 + (i % 2);
...
/// Compare all STREAM kernels
bool test_stream_comparison() {
    std::cout << "\n=== Test: STREAM Kernel Comparison ===" << std::endl;
    ...
    // Copy
    {
        ...
        for (int i = 0; i < STREAM_SIZE; ++i) {
            uint32_t row = 100 + (i / 32);
            uint32_t col = (i % 32) * CACHE_LINE_BYTES;
            harness.submit_read(make_address(read_banks[i % 4], row, col));
            harness.submit_write(make_address(write_banks[i % 4], row, col));
        }
    ...
    // Add
    {
        ...
        for (int i = 0; i < STREAM_SIZE; ++i) {
            uint32_t row = 100 + (i / 32);
            uint32_t col = (i % 32) * CACHE_LINE_BYTES;
            harness.submit_read(make_address(2 + (i % 2), row, col));
            harness.submit_read(make_address(4 + (i % 2), row, col));
            harness.submit_write(make_address(i % 2, row, col));
        }
        harness.run_until_complete();
    ...
    std::cout << "\n--- STREAM Comparison Results ---" << std::endl;
...
int main(int argc, char* argv[]) {
    std::cout << "================================================" << std::endl;
    std::cout << "LPDDR5 Pattern: STREAM Benchmark" << std::endl;
    ...
    pass &= test_stream_scale();
    pass &= test_stream_add();
    pass &= test_stream_triad();
    pass &= test_stream_comparison();

    // Export trace for visualization (Triad pattern)
    if (export_trace) {
        std::cout << "\n=== Trace Export ===" << std::endl;
        LPDDR5Harness harness(single_channel_config());

        // Generate trace for first 64 Triad iterations
        for (int i = 0; i < 64; ++i) {
            uint32_t row = 100 + (i / 32);
            uint32_t col = (i % 32) * CACHE_LINE_BYTES;
            harness.submit_read(make_address(2 + (i % 2), row, col));
