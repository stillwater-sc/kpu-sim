#!/usr/bin/env python3
"""
GDDR7 Memory Controller Trace Validator

Validates Chrome Trace JSON files against GDDR7 trace invariants.
...
# GDDR7-32000 timing parameters (cycles at 2.0 GHz CK with PAM3)
TIMING = {
    'tRCDRD': 14,  # Row address to column address delay (read)
    'tRCDWR': 14,  # Row address to column address delay (write)
    'tRP': 14,     # Row precharge time
    'tRL': 16,     # CAS read latency
    'tRRD_L': 4,   # ACT-to-ACT same bank group
    'tRRD_S': 3,   # ACT-to-ACT different bank group
    'tFAW': 12,    # Four activate window
    'tRTW': 12,    # Read-to-write turnaround
    'tWTR_L': 5,   # Write-to-read turnaround (same BG)
    'tWTR_S': 3,   # Write-to-read turnaround (diff BG)
    'tCCD_L': 3,   # CAS-to-CAS same bank group
    'tCCD_S': 2,   # CAS-to-CAS different bank group
}

# GDDR7 has 4 bank groups with 4 banks each (16 banks total)
BANKS_PER_GROUP = 4
...
def get_bank_group(bank: int) -> int:
    """GDDR7: 4 bank groups with 4 banks each."""
    return bank // BANKS_PER_GROUP


class TraceValidator:
    """Validates GDDR7 memory controller traces against invariants."""
    ...
    def validate(self) -> bool:
        ...
        self._check_inv104_tccd_constraint()
    ...
    def _check_inv100_trcd_constraint(self):
        ...
        for txn_id, txn in self.transactions.items():
            ...
            if activate and data_op:
                ...
                trcd = TIMING['tRCDRD'] if txn.has_read else TIMING['tRCDWR']
                if gap < trcd:
                    self.violations.append(Violation(
                        invariant='INV-100',
                        severity=Severity.WARNING,
                        message=f"txn_id={txn_id}: tRCD violation - gap={gap} cycles, required={trcd}",
                        txn_id=txn_id,
                        bank=activate.bank,
                        fix_hint=f"Wait at least tRCD={trcd} cycles after ACTIVATE"
                    ))
    ...
    def _check_inv102_trrd_constraint(self):
        ...
        for i in range(1, len(activates)):
            ...
            # GDDR7: bank group = bank // 4
            prev_bg = get_bank_group(prev.bank)
            curr_bg = get_bank_group(curr.bank)
    ...
    def _check_inv104_tccd_constraint(self):
        """INV-104: Minimum time between CAS commands (tCCD_L/tCCD_S)."""
        cas_cmds = [e for e in self.events if 'READ' in e.name or 'WRITE' in e.name]
        cas_cmds.sort(key=lambda e: e.cycle_issue)

        for i in range(1, len(cas_cmds)):
            prev = cas_cmds[i-1]
            curr = cas_cmds[i]
            gap = curr.cycle_issue - prev.cycle_issue
            ...
            if gap < required:
                self.violations.append(Violation(
                    invariant='INV-104',
                    severity=Severity.WARNING,
                    message=f"tCCD violation: CAS gap={gap} cycles between banks {prev.bank}->{curr.bank}, "
                           f"required={required} ({'same BG' if same_bg else 'diff BG'})",
    ...
    def print_report(self, json_output: bool = False):
        ...
        print(f"\n{'='*60}")
        print(f"GDDR7 Trace Validation Report")
        print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Validate GDDR7 memory controller traces against invariants'
    )
