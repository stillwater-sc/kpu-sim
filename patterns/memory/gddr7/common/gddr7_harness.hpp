// patterns/memory/gddr7/common/gddr7_harness.hpp
//
// GDDR7-specific test harness for pattern validation
//
...
inline fs::path get_trace_dir(const std::string& category) {
    ...
    fs::path trace_dir = root / "traces" / "memory" / "gddr7" / category;
...
/// GDDR7-specific test harness for memory controller patterns
///
/// Provides:
...
class GDDR7Harness {
public:
    explicit GDDR7Harness(const GDDR7MemoryController::Config& config)
        : config_(config)
        , mc_(std::make_unique<GDDR7MemoryController>(config))
    ...
    /// Get statistics
    const Statistics& stats() const { return mc_->gddr7_stats(); }

    /// Get violations
    const std::vector<InvariantViolation>& violations() const {
        return mc_->gddr7_violations();
    }
    ...
    void print_violations(std::ostream& os = std::cerr) const {
        if (mc_->has_violations()) {
            ...
            for (const auto& v : mc_->gddr7_violations()) {
                os << "  Cycle " << v.cycle
                   << ": [" << v.invariant_id << "] "
                   << v.message
    ...
    GDDR7MemoryController& memory_controller() { return *mc_; }
    const GDDR7MemoryController& memory_controller() const { return *mc_; }

protected:
    GDDR7MemoryController::Config config_;
    std::unique_ptr<GDDR7MemoryController> mc_;
