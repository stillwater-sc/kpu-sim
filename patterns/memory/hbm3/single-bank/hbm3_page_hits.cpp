// patterns/memory/hbm3/single-bank/hbm3_page_hits.cpp
//
// HBM3 single bank page hit pattern - sequential reads to same row
//
...
int main() {
    std::cout << "=== HBM3 Single Bank Page Hits Pattern ===" << std::endl;
    ...
    // Export trace
    std::string trace_path = make_trace_path("single-bank", "hbm3_page_hits_trace.json");
    harness.export_trace(trace_path);

    std::cout << "\nPASS: HBM3 page hits pattern completed successfully" << std::endl;
    return 0;
}
