// patterns/memory/hbm3/full-bank/hbm3_bank_distribution.cpp
//
// HBM3 Bank Distribution Analysis
...
void test_sequential_addresses() {
    ...
    constexpr int NUM_ACCESSES = 512;
    constexpr int STRIDE = 64;  // Cache line stride
    std::vector<int> bank_counts(512, 0);
    ...
    for (int i = 0; i < NUM_ACCESSES; ++i) {
        ...
        uint8_t channel = (addr >> 5) & 0xF;       // bits [8:5]
        uint8_t pc = (addr >> 9) & 0x1;            // bit [9]
        uint8_t bank = (addr >> 10) & 0xF;         // bits [13:10]
    ...
    print_bank_histogram(bank_counts, 16, 2, 16);

    std::string trace_path = make_trace_path("full-bank", "hbm3_sequential_distribution_trace.json");
...
void test_strided_addresses() {
    ...
    for (int i = 0; i < NUM_ACCESSES; ++i) {
        ...
        // Decode address
        uint8_t channel = (addr >> 5) & 0xF;
        uint8_t pc = (addr >> 9) & 0x1;
        uint8_t bank = (addr >> 10) & 0xF;
    ...
    std::string trace_path = make_trace_path("full-bank", "hbm3_strided_distribution_trace.json");
    harness.export_trace(trace_path);
}
...
void test_bank_spread_pattern() {
    ...
    std::cout << "Deliberately spreading accesses across all 512 banks" << std::endl;
    ...
    harness.run_until_complete(200000);
    ...
    std::string trace_path = make_trace_path("full-bank", "hbm3_bank_spread_distribution_trace.json");
    harness.export_trace(trace_path);
}
...
void test_hot_bank_pattern() {
    ...
    std::string trace_path = make_trace_path("full-bank", "hbm3_hot_bank_distribution_trace.json");
    harness.export_trace(trace_path);
}

int main() {
    std::cout << "=== HBM3 Bank Distribution Analysis ===" << std::endl;
    std::cout << "\nThis test demonstrates how different access patterns" << std::endl;
    std::cout << "distribute across HBM3's 512 banks and the impact on" << std::endl;
    std::cout << "performance and parallelism." << std::endl;
    std::cout << "\nHBM3 Address Mapping:" << std::endl;
    std::cout << "  [row:14 | bank:4 | col:5 | pc:1 | channel:4 | offset:5]" << std::endl;
    std::cout << "  Total: 16 channels × 2 PCs × 16 banks = 512 banks" << std::endl;
    ...
    std::cout << "\nTraces exported to: traces/memory/hbm3/full-bank/" << std::endl;
    return 0;
}
