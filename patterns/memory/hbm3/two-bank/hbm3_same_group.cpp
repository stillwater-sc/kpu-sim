// patterns/memory/hbm3/two-bank/hbm3_same_group.cpp
//
// HBM3 two-bank same bank group pattern
...
int main() {
    std::cout << "=== HBM3 Two Bank Same Group Pattern ===" << std::endl;
    ...
    // Export trace
    std::string trace_path = make_trace_path("two-bank", "hbm3_same_group_trace.json");
    harness.export_trace(trace_path);

    std::cout << "\nPASS: HBM3 same bank group pattern completed successfully" << std::endl;
    return 0;
}
