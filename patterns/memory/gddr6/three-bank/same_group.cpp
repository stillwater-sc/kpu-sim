// patterns/memory/gddr6/three-bank/same_group.cpp
//
// GDDR6 Three Banks Same Group Pattern
// Tests bank group saturation with tRRD_L constraints
//
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== GDDR6 Three Banks Same Group Pattern ===" << std::endl;
    std::cout << "Round-robin accesses to banks 0, 1, 2 (all in Bank Group 0)" << std::endl;
    std::cout << "Tests bank group saturation and tRRD_L accumulation" << std::endl;
    ...
    std::cout << "  All same group: yes (constrained by tRRD_L=" << tRRD_L << ")" << std::endl;

    if (export_trace) {
        std::string trace_file = make_trace_path("three-bank", "same_group_trace.json");
    ...
    if (run_fidelity) {
        ...
        auto workload = make_three_banks_same_group_workload(CHANNEL, ROW, NUM_PER_BANK);
