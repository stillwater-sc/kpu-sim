// patterns/memory/gddr6/common/gddr6_harness.hpp
//
// GDDR6-specific test harness for pattern validation
//
...
inline fs::path get_trace_dir(const std::string& category) {
    ...
    fs::path trace_dir = root / "traces" / "memory" / "gddr6" / category;
...
/// GDDR6-specific test harness for memory controller patterns
///
/// Provides:
...
class GDDR6Harness {
public:
    explicit GDDR6Harness(const GDDR6MemoryController::Config& config)
        : config_(config)
        , mc_(std::make_unique<GDDR6MemoryController>(config))
    ...
    /// Get statistics
    const Statistics& stats() const { return mc_->gddr6_stats(); }

    /// Get violations
    const std::vector<InvariantViolation>& violations() const {
        return mc_->gddr6_violations();
    }
    ...
    void print_stats(std::ostream& os = std::cout) const {
        const auto& s = mc_->gddr6_stats();
        os << "\n=== GDDR6 Memory Controller Statistics ===" << std::endl;
        ...
        os << "  Channel A accesses: " << s.channel_a_accesses << std::endl;
        os << "  Channel B accesses: " << s.channel_b_accesses << std::endl;
        os << "  Channel balance:    " << std::setprecision(1)
           << (s.channel_balance() * 100.0) << "% Ch A" << std::endl;
    ...
    void print_calibration_data(std::ostream& os = std::cout) const {
        const auto& s = mc_->gddr6_stats();
        os << "\n=== GDDR6 Calibration Data ===" << std::endl;
        os << "Page hit latency:      " << std::fixed << std::setprecision(2)
           << s.avg_page_hit_latency() << " cycles (factor: "
           << s.page_hit_factor() << ")" << std::endl;
        os << "Page empty latency:    " << s.avg_page_empty_latency() << " cycles (factor: "
           << s.page_empty_factor() << ")" << std::endl;
        os << "Page conflict latency: " << s.avg_page_conflict_latency() << " cycles (factor: "
           << s.page_conflict_factor() << ")" << std::endl;
        os << "================================\n" << std::endl;
    }
    ...
    void print_violations(std::ostream& os = std::cerr) const {
        if (mc_->has_violations()) {
            ...
            for (const auto& v : mc_->gddr6_violations()) {
                os << "  Cycle " << v.cycle
                   << ": [" << v.invariant_id << "] "
                   << v.message
    ...
    /// @param clock_ghz Clock frequency for time conversion (default: 2.0 GHz for GDDR6-16000)
    ...
    bool export_resources(const std::string& filename, double clock_ghz = 2.0) {
        tracker_->finalize(mc_->current_cycle());
    ...
    GDDR6MemoryController& memory_controller() { return *mc_; }
    const GDDR6MemoryController& memory_controller() const { return *mc_; }

protected:
    GDDR6MemoryController::Config config_;
    std::unique_ptr<GDDR6MemoryController> mc_;
