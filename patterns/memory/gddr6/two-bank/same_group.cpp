// patterns/memory/gddr6/two-bank/same_group.cpp
//
// GDDR6 Two Banks Same Group Pattern
// Tests tRRD_L and tCCD_L constraints (same bank group timing)
//
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== GDDR6 Two Banks Same Group Pattern ===" << std::endl;
    std::cout << "Alternating accesses to banks 0 and 1 (both in Bank Group 0)" << std::endl;
    std::cout << "Tests tRRD_L (ACT-to-ACT) and tCCD_L (CAS-to-CAS) constraints" << std::endl;
    ...
    std::cout << "\nSubmitting " << (NUM_PER_BANK * 2) << " reads alternating between banks "
              << (int)BANK_A << " and " << (int)BANK_B << " (same bank group)" << std::endl;
    ...
    // Verify same bank group constraint was exercised
    ...
    if (export_trace) {
        std::string trace_file = make_trace_path("two-bank", "same_group_trace.json");
    ...
    if (run_fidelity) {
        ...
        auto workload = make_two_banks_same_group_workload(CHANNEL, ROW, NUM_PER_BANK);
