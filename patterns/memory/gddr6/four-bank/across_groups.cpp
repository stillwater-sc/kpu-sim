// patterns/memory/gddr6/four-bank/across_groups.cpp
//
// GDDR6 Four Banks Across Groups Pattern
// Tests maximum bank-level parallelism with one bank per group
//
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== GDDR6 Four Banks Across Groups Pattern ===" << std::endl;
    std::cout << "Accesses to banks 0, 4, 8, 12 (one per bank group)" << std::endl;
    std::cout << "Tests maximum parallelism with tRRD_S/tCCD_S" << std::endl;
    ...
    std::cout << "  All different groups: yes (uses tRRD_S=" << tRRD_S << ", tCCD_S=" << tCCD_S << ")" << std::endl;
    std::cout << "  Maximum parallelism achieved - no bank group contention" << std::endl;

    if (export_trace) {
        std::string trace_file = make_trace_path("four-bank", "across_groups_trace.json");
    ...
    if (run_fidelity) {
        ...
        auto workload = make_four_banks_across_groups_workload(CHANNEL, ROW, NUM_PER_BANK);
