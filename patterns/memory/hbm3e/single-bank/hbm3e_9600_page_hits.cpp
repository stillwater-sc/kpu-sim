// patterns/memory/hbm3e/single-bank/hbm3e_9600_page_hits.cpp
//
// HBM3E-9600 single bank page hit pattern - sequential reads to same row
// Uses HBM3E-9600 timing @ 4.8 GHz CK (1.229 TB/s peak bandwidth)
//
...
int main() {
    std::cout << "=== HBM3E-9600 Single Bank Page Hits Pattern ===" << std::endl;
    std::cout << "Testing sequential reads to same row (page hit scenario)" << std::endl;

    // Use single pseudo-channel HBM3E-9600 config for focused testing
    ...
    // Export trace
    std::string trace_path = make_trace_path("single-bank", "hbm3e_9600_page_hits_trace.json");
    harness.export_trace(trace_path);

    std::cout << "\nPASS: HBM3E-9600 page hits pattern completed successfully" << std::endl;
    return 0;
}
