// patterns/memory/ddr5/common/ddr5_harness.hpp
//
// DDR5-specific test harness for pattern validation
//
...
inline fs::path get_trace_dir(const std::string& category) {
    ...
    fs::path trace_dir = root / "traces" / "memory" / "ddr5" / category;
...
/// DDR5-specific test harness for memory controller patterns
///
/// Provides:
...
class DDR5Harness {
public:
    explicit DDR5Harness(const DDR5MemoryController::Config& config)
        : config_(config)
        , mc_(std::make_unique<DDR5MemoryController>(config))
    ...
    /// Get statistics
    const Statistics& stats() const { return mc_->ddr5_stats(); }

    /// Get violations
    const std::vector<InvariantViolation>& violations() const {
        return mc_->ddr5_violations();
    }
    ...
    void print_violations(std::ostream& os = std::cerr) const {
        if (mc_->has_violations()) {
            ...
            for (const auto& v : mc_->ddr5_violations()) {
                os << "  Cycle " << v.cycle
                   << ": [" << v.invariant_id << "] "
                   << v.message
    ...
    bool export_trace(const std::string& filename, double clock_ghz = 2.4) {
    ...
    bool export_resources(const std::string& filename, double clock_ghz = 2.4) {
        tracker_->finalize(mc_->current_cycle());
    ...
    DDR5MemoryController& memory_controller() { return *mc_; }
    const DDR5MemoryController& memory_controller() const { return *mc_; }

protected:
    DDR5MemoryController::Config config_;
    std::unique_ptr<DDR5MemoryController> mc_;
