// patterns/memory/hbm2e/common/hbm2e_harness.hpp
//
// HBM2E-specific test harness for pattern validation
//
...
inline fs::path get_trace_dir(const std::string& category) {
    ...
    fs::path trace_dir = root / "traces" / "memory" / "hbm2e" / category;
...
/// HBM2E variant enumeration
enum class HBM2EVariant {
    HBM2E_3200,  // 3.2 Gbps @ 1.6 GHz
    HBM2E_3600   // 3.6 Gbps @ 1.8 GHz
};

/// Get clock frequency for variant
inline double get_clock_ghz(HBM2EVariant variant) {
    switch (variant) {
        case HBM2EVariant::HBM2E_3200: return CLOCK_GHZ_3200;
        case HBM2EVariant::HBM2E_3600: return CLOCK_GHZ_3600;
    }
    return CLOCK_GHZ_3600;  // Default
}

/// Get variant name string
inline std::string get_variant_name(HBM2EVariant variant) {
    switch (variant) {
        case HBM2EVariant::HBM2E_3200: return "HBM2E-3200";
        case HBM2EVariant::HBM2E_3600: return "HBM2E-3600";
    }
    return "HBM2E";
}

/// Get peak bandwidth for variant
inline double get_peak_bandwidth(HBM2EVariant variant) {
    switch (variant) {
        case HBM2EVariant::HBM2E_3200: return HBM2E_3200_BANDWIDTH;
        case HBM2EVariant::HBM2E_3600: return HBM2E_3600_BANDWIDTH;
    }
    return HBM2E_3600_BANDWIDTH;
}

/// HBM2E-specific test harness for memory controller patterns
///
/// Provides:
...
/// - Trace export with correct clock frequency
class HBM2EHarness {
public:
    explicit HBM2EHarness(const HBM2MemoryController::Config& config,
                          HBM2EVariant variant = HBM2EVariant::HBM2E_3600)
        : config_(config)
        , variant_(variant)
        , mc_(std::make_unique<HBM2MemoryController>(config))
    ...
    virtual ~HBM2EHarness() = default;

    // ========================================================================
    // Variant Information
    // ========================================================================

    HBM2EVariant variant() const { return variant_; }
    std::string variant_name() const { return get_variant_name(variant_); }
    double clock_ghz() const { return get_clock_ghz(variant_); }
    double peak_bandwidth() const { return get_peak_bandwidth(variant_); }
    ...
    /// Get statistics
    const Statistics& stats() const { return mc_->hbm2_stats(); }

    /// Get violations
    const std::vector<InvariantViolation>& violations() const {
        return mc_->hbm2_violations();
    }
    ...
    void print_stats(std::ostream& os = std::cout) const {
        const auto& s = mc_->hbm2_stats();
        os << "\n=== " << variant_name() << " Memory Controller Statistics ===" << std::endl;
        os << "  Configuration:      " << variant_name() << " @ " << clock_ghz() << " GHz" << std::endl;
        os << "  Peak bandwidth:     " << peak_bandwidth() << " GB/s" << std::endl;
        ...
        // Print per-channel utilization
        os << "  Channel utilization:" << std::endl;
    ...
    void print_calibration_data(std::ostream& os = std::cout) const {
        const auto& s = mc_->hbm2_stats();
        os << "\n=== " << variant_name() << " Calibration Data ===" << std::endl;
    ...
    void print_violations(std::ostream& os = std::cerr) const {
        if (mc_->has_violations()) {
            ...
            for (const auto& v : mc_->hbm2_violations()) {
                os << "  Cycle " << v.cycle
                   << ": [" << v.invariant_id << "] "
                   << v.message
    ...
    bool export_resources(const std::string& filename) {
        tracker_->finalize(mc_->current_cycle());
    ...
    HBM2MemoryController& memory_controller() { return *mc_; }
    const HBM2MemoryController& memory_controller() const { return *mc_; }

protected:
    HBM2MemoryController::Config config_;
    HBM2EVariant variant_;
    std::unique_ptr<HBM2MemoryController> mc_;
