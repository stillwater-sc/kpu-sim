// patterns/memory/hbm2e/single-bank/hbm2e_3600_page_conflicts.cpp
//
// HBM2E-3600 single bank page conflict pattern - alternating rows
...
int main() {
    std::cout << "=== HBM2E-3600 Single Bank Page Conflicts Pattern ===" << std::endl;
    std::cout << "Testing alternating row accesses (worst-case page conflict scenario)" << std::endl;

    // Use single pseudo-channel HBM2E-3600 config
    ...
    // Export trace
    std::string trace_path = make_trace_path("single-bank", "hbm2e_3600_page_conflicts_trace.json");
    harness.export_trace(trace_path);

    std::cout << "\nPASS: HBM2E-3600 page conflicts pattern completed successfully" << std::endl;
    return 0;
}
