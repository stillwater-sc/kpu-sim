// patterns/memory/hbm2e/single-bank/hbm2e_3200_page_hits.cpp
//
// HBM2E-3200 single bank page hit pattern - sequential reads to same row
// Uses HBM2E-3200 timing @ 1.6 GHz CK (409.6 GB/s peak bandwidth)
//
...
int main() {
    std::cout << "=== HBM2E-3200 Single Bank Page Hits Pattern ===" << std::endl;
    std::cout << "Testing sequential reads to same row (page hit scenario)" << std::endl;

    // Use single pseudo-channel HBM2E-3200 config for focused testing
    auto config = single_pc_config_3200();
    HBM2EHarness harness(config, HBM2EVariant::HBM2E_3200);
    ...
    // Export trace
    std::string trace_path = make_trace_path("single-bank", "hbm2e_3200_page_hits_trace.json");
    harness.export_trace(trace_path);

    std::cout << "\nPASS: HBM2E-3200 page hits pattern completed successfully" << std::endl;
    return 0;
}
