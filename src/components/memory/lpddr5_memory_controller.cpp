...
BankState LPDDR5MemoryController::get_bank_state(uint8_t channel, uint8_t bank) const {
    if (channel >= config_.num_channels || bank >= 16) {
...
bool LPDDR5MemoryController::is_bank_idle(uint8_t channel, uint8_t bank) const {
    return get_bank_state(channel, bank) == BankState::IDLE;
}
...
bool LPDDR5MemoryController::is_row_open(uint8_t channel, uint8_t bank, uint32_t row) const {
    ...
    return (b.state == BankState::ACTIVE ||
            b.state == BankState::READING ||
            b.state == BankState::WRITING) &&
           b.open_row == row;
}
    ...
    uint32_t& row, uint32_t& col) const
{
    ...
    uint32_t col_bits = config_.col_bits;
    uint32_t bank_bits = config_.bank_bits;
    uint32_t channel_bits = (config_.num_channels > 1) ? 1 : 0;
...
uint32_t LPDDR5MemoryController::burst_cycles() const {
    return (config_.burst_length == BurstLength::BL16) ?
           config_.timing.tBurst_BL16 : config_.timing.tBurst_BL32;
}
...
bool LPDDR5MemoryController::can_refresh(uint8_t channel, uint8_t bank) const {
    ...
    return channels_[channel].banks[bank].state == BankState::IDLE;
}
...
void LPDDR5MemoryController::do_activate(uint8_t channel, uint8_t bank, uint32_t row) {
    ...
    // Trace the operation
    trace_bank_state_change(channel, bank, BankState::ACTIVATING, "ACTIVATE row " + std::to_string(row));
...
void LPDDR5MemoryController::do_read(uint8_t channel, uint8_t bank, MemoryRequest& req) {
    ...
    trace_bank_state_change(channel, bank, BankState::READING, "READ burst");
...
void LPDDR5MemoryController::do_write(uint8_t channel, uint8_t bank, MemoryRequest& req) {
    ...
    trace_bank_state_change(channel, bank, BankState::WRITING, "WRITE burst");
...
void LPDDR5MemoryController::do_precharge(uint8_t channel, uint8_t bank) {
    ...
    // Trace the operation
    trace_bank_state_change(channel, bank, BankState::PRECHARGING, "PRECHARGE");
...
void LPDDR5MemoryController::do_refresh(uint8_t channel, uint8_t bank) {
    ...
    // Trace the operation
    trace_bank_state_change(channel, bank, BankState::REFRESHING, "REFRESH");
    ...
    for (uint8_t ch = 0; ch < config_.num_channels; ++ch) {
        for (uint8_t b = 0; b < 16; ++b) {
            ...
            if (current_cycle_ >= bank.state_until) {
                switch (bank.state) {
                    case BankState::ACTIVATING:
                        ...
                        trace_bank_state_change(ch, b, BankState::ACTIVE, "tRCD complete");
                        break;
                    case BankState::PRECHARGING:
                        ...
                        trace_bank_state_change(ch, b, BankState::IDLE, "tRP complete");
                        break;
                    case BankState::REFRESHING:
                        bank.state = BankState::IDLE;
                        trace_bank_state_change(ch, b, BankState::IDLE, "tRFCpb complete");
                        break;
                    default:
                        break;
                }
            ...
            if (current_cycle_ >= bank.burst_end) {
                if (bank.state == BankState::READING || bank.state == BankState::WRITING) {
                    bank.state = BankState::ACTIVE;
                    trace_bank_state_change(ch, b, BankState::ACTIVE, "burst complete");
                }
            }
        }
    }
...
void LPDDR5MemoryController::check_bank_invariants(uint8_t channel, uint8_t bank) {
    ...
    if (b.state == BankState::ACTIVE && current_cycle_ < b.state_until) {
    ...
    if (b.state == BankState::PRECHARGING) {
    ...
    if (b.state == BankState::PRECHARGING && b.last_write_cmd > 0) {
        uint64_t write_done = b.last_write_cmd + timing.tWL + burst_cycles();
    ...
    if (b.state == BankState::PRECHARGING && b.last_read_cmd > 0) {
...
void LPDDR5MemoryController::trace_bank_state_change(
    uint8_t channel, uint8_t bank, BankState new_state, const std::string& reason)
{
