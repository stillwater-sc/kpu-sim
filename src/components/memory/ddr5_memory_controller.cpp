...
void DDR5MemoryController::set_refresh_mode(RefreshMode mode) {
    refresh_mode_ = mode;
...
void DDR5MemoryController::set_refresh_interval(uint64_t cycles) {
    refresh_interval_ = cycles;
    last_interval_refresh_ = current_cycle_;
}

uint64_t DDR5MemoryController::cycles_until_deadline(uint8_t channel, uint8_t bank) const {
    if (channel >= ddr5_config_.num_channels || bank >= 32) {
        return UINT64_MAX;
    }
    ...
    // Deadline is 32 * tREFI after last refresh (32 banks per channel)
    uint64_t deadline = b.last_refresh + 32 * timing.tREFI;
...
bool DDR5MemoryController::refresh_pending(uint8_t channel, uint8_t bank) const {
    if (channel >= ddr5_config_.num_channels || bank >= 32) {
        return false;
    }
...
uint32_t DDR5MemoryController::refresh_debt(uint8_t channel, uint8_t bank) const {
    if (channel >= ddr5_config_.num_channels || bank >= 32) {
        return 0;
    }
...
bool DDR5MemoryController::inject_refresh(uint8_t channel, int8_t bank) {
    if (channel >= ddr5_config_.num_channels) {
        return false;
    }
    ...
    if (bank == -1) {
        ...
        if (can_refresh(channel, next_bank)) {
            ...
            channels_[channel].next_refresh_bank = (next_bank + 1) % 32;
            return true;
        }
        return false;
    }
