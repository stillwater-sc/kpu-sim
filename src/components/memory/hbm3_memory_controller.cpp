// ============================================================================
// src/components/memory/hbm3_memory_controller.cpp
// HBM3 Memory Controller implementation with formal invariant checking
// ============================================================================
...
HBM3MemoryController::HBM3MemoryController(const Config& config)
    : hbm3_config_(config)
{
    ...
    // Initialize interface config from HBM3 config
    interface_config_.fidelity = sw::kpu::SimulationFidelity::CYCLE_ACCURATE;
    interface_config_.technology = sw::kpu::MemoryTechnology::HBM3;
...
HBM3MemoryController::HBM3MemoryController(const sw::kpu::MemoryControllerConfig& config)
    : HBM3MemoryController(Config{})
{
    // Copy relevant fields from interface config
    hbm3_config_.num_channels = config.num_channels;
    hbm3_config_.queue_depth = config.queue_depth;
    hbm3_config_.row_bits = config.row_bits;
    hbm3_config_.col_bits = config.col_bits;
    hbm3_config_.bank_bits = config.bank_bits;
    hbm3_config_.channel_bits = config.channel_bits;

    // Copy timing parameters
    hbm3_config_.timing.tRCD = config.timing.tRCD;
    hbm3_config_.timing.tRP = config.timing.tRP;
    hbm3_config_.timing.tRAS = config.timing.tRAS;
    hbm3_config_.timing.tRC = config.timing.tRC;
    hbm3_config_.timing.tRL = config.timing.tCL;
    hbm3_config_.timing.tWL = config.timing.tWL;
    hbm3_config_.timing.tWR = config.timing.tWR;
    hbm3_config_.timing.tRTP = config.timing.tRTP;
    hbm3_config_.timing.tRRD_L = config.timing.tRRD_L;
    hbm3_config_.timing.tRRD_S = config.timing.tRRD_S;
    hbm3_config_.timing.tCCD_L = config.timing.tCCD_L;
    hbm3_config_.timing.tCCD_S = config.timing.tCCD_S;
    hbm3_config_.timing.tWTR_L = config.timing.tWTR_L;
    hbm3_config_.timing.tWTR_S = config.timing.tWTR_S;
    hbm3_config_.timing.tRTW = config.timing.tRTW;
    hbm3_config_.timing.tFAW = config.timing.tFAW;
...
std::optional<uint64_t> HBM3MemoryController::submit_read(
...
std::optional<uint64_t> HBM3MemoryController::submit_write(
...
void HBM3MemoryController::tick() {
...
void HBM3MemoryController::drain() {
...
void HBM3MemoryController::reset() {
...
bool HBM3MemoryController::has_pending() const {
    return !pending_queue_.empty() || !active_requests_.empty();
}

bool HBM3MemoryController::can_accept() const {
    return pending_queue_.size() < hbm3_config_.queue_depth;
}

size_t HBM3MemoryController::pending_count() const {
    return pending_queue_.size() + active_requests_.size();
}

// Convert HBM3 BankState to IMemoryController::BankState
static sw::kpu::IMemoryController::BankState to_interface_state(hbm3::BankState state) {
    switch (state) {
        case hbm3::BankState::IDLE:        return sw::kpu::IMemoryController::BankState::IDLE;
        case hbm3::BankState::ACTIVATING:  return sw::kpu::IMemoryController::BankState::ACTIVATING;
        case hbm3::BankState::ACTIVE:      return sw::kpu::IMemoryController::BankState::ACTIVE;
        case hbm3::BankState::READING:     return sw::kpu::IMemoryController::BankState::READING;
        case hbm3::BankState::WRITING:     return sw::kpu::IMemoryController::BankState::WRITING;
        case hbm3::BankState::PRECHARGING: return sw::kpu::IMemoryController::BankState::PRECHARGING;
        case hbm3::BankState::REFRESHING:  return sw::kpu::IMemoryController::BankState::REFRESHING;
        default:                           return sw::kpu::IMemoryController::BankState::IDLE;
    }
}

sw::kpu::IMemoryController::BankState HBM3MemoryController::get_bank_state(
    uint8_t channel, uint8_t bank) const
{
    // Map flat bank index to pseudo-channel + bank
    uint8_t pc = bank / 16;
    uint8_t local_bank = bank % 16;
    return to_interface_state(get_hbm3_bank_state(channel, pc, local_bank));
}

hbm3::BankState HBM3MemoryController::get_hbm3_bank_state(uint8_t channel, uint8_t pc, uint8_t bank) const {
    if (channel >= hbm3_config_.num_channels || pc >= 2 || bank >= 16) {
        return hbm3::BankState::IDLE;
    }
    return channels_[channel].pseudo_channels[pc].banks[bank].state;
}

bool HBM3MemoryController::is_bank_idle(uint8_t channel, uint8_t pc, uint8_t bank) const {
    return get_hbm3_bank_state(channel, pc, bank) == hbm3::BankState::IDLE;
}

bool HBM3MemoryController::is_row_open(uint8_t channel, uint8_t bank, uint32_t row) const {
    // Map flat bank index to pseudo-channel + bank
    uint8_t pc = bank / 16;
    uint8_t local_bank = bank % 16;

    if (channel >= hbm3_config_.num_channels || pc >= 2 || local_bank >= 16) {
        return false;
    }
    const Bank& b = channels_[channel].pseudo_channels[pc].banks[local_bank];
    return (b.state == hbm3::BankState::ACTIVE ||
            b.state == hbm3::BankState::READING ||
            b.state == hbm3::BankState::WRITING) &&
           b.open_row == row;
}

void HBM3MemoryController::reset_stats() {
    stats_ = Statistics{};
    interface_stats_.reset();
}

void HBM3MemoryController::sync_interface_stats() {
...
void HBM3MemoryController::sync_interface_violations() {
    interface_violations_.clear();
...
void HBM3MemoryController::decode_address(
    uint64_t address, uint8_t& channel, uint8_t& pc, uint8_t& bank,
    uint32_t& row, uint32_t& col) const
{
    // HBM3 address mapping:
    // [row | bank | col | pseudo_channel | channel | byte_offset]
    uint32_t byte_offset_bits = 5;  // 32-byte minimum access
    uint32_t col_bits = hbm3_config_.col_bits;
    uint32_t bank_bits = hbm3_config_.bank_bits;
    uint32_t pc_bits = hbm3_config_.pc_bits;
    uint32_t channel_bits = hbm3_config_.channel_bits;
    ...
    // Extract row
    row = addr & ((1 << hbm3_config_.row_bits) - 1);
}

uint64_t HBM3MemoryController::encode_address(uint8_t channel, uint8_t pc, uint8_t bank,
                                               uint32_t row, uint32_t col) const
{
...
uint32_t HBM3MemoryController::burst_cycles() const {
    // HBM3 BL8: 4 CK cycles for burst (DDR)
    return hbm3_config_.timing.tBurst;
}
...
bool HBM3MemoryController::check_tFAW(uint8_t channel, uint8_t pc) const {
...
void HBM3MemoryController::record_activate(uint8_t channel, uint8_t pc) {
...
bool HBM3MemoryController::can_activate(uint8_t channel, uint8_t pc, uint8_t bank, uint64_t cycle) const {
    const auto& pseudo_ch = channels_[channel].pseudo_channels[pc];
...
bool HBM3MemoryController::can_read(uint8_t channel, uint8_t pc, uint8_t bank, uint64_t cycle) const {
    const auto& pseudo_ch = channels_[channel].pseudo_channels[pc];
    const Bank& b = pseudo_ch.banks[bank];
    const auto& timing = hbm3_config_.timing;

    // Bank must be ACTIVE
    if (b.state != hbm3::BankState::ACTIVE) {
        return false;
    }

    // tRCD must have elapsed
    if (cycle < b.state_until) {
        return false;
    }
...
bool HBM3MemoryController::can_write(uint8_t channel, uint8_t pc, uint8_t bank, uint64_t cycle) const {
    const auto& pseudo_ch = channels_[channel].pseudo_channels[pc];
    const Bank& b = pseudo_ch.banks[bank];
    const auto& timing = hbm3_config_.timing;
...
bool HBM3MemoryController::can_precharge(uint8_t channel, uint8_t pc, uint8_t bank, uint64_t cycle) const {
    const auto& pseudo_ch = channels_[channel].pseudo_channels[pc];
    const Bank& b = pseudo_ch.banks[bank];
    const auto& timing = hbm3_config_.timing;
...
bool HBM3MemoryController::can_refresh(uint8_t channel, uint8_t pc, uint8_t bank) const {
    return channels_[channel].pseudo_channels[pc].banks[bank].state == hbm3::BankState::IDLE;
}
...
void HBM3MemoryController::do_activate(uint8_t channel, uint8_t pc, uint8_t bank, uint32_t row, uint64_t request_id) {
    auto& pseudo_ch = channels_[channel].pseudo_channels[pc];
    Bank& b = pseudo_ch.banks[bank];
    const auto& timing = hbm3_config_.timing;

    b.state = hbm3::BankState::ACTIVATING;
    b.open_row = row;
...
void HBM3MemoryController::do_read(uint8_t channel, uint8_t pc, uint8_t bank, MemoryRequest& req) {
    ...
    trace_bank_state_change(channel, pc, bank, hbm3::BankState::READING, "READ burst");
...
void HBM3MemoryController::do_write(uint8_t channel, uint8_t pc, uint8_t bank, MemoryRequest& req) {
    ...
    trace_bank_state_change(channel, pc, bank, hbm3::BankState::WRITING, "WRITE burst");
...
void HBM3MemoryController::do_precharge(uint8_t channel, uint8_t pc, uint8_t bank) {
    ...
    b.state = hbm3::BankState::PRECHARGING;
...
void HBM3MemoryController::do_refresh(uint8_t channel, uint8_t pc, uint8_t bank) {
    ...
    b.state = hbm3::BankState::REFRESHING;
...
void HBM3MemoryController::update_bank_states() {
    for (uint8_t ch = 0; ch < hbm3_config_.num_channels; ++ch) {
        for (uint8_t pc = 0; pc < hbm3_config_.pseudo_channels_per_channel; ++pc) {
            for (uint8_t b = 0; b < 16; ++b) {
                ...
                if (current_cycle_ >= bank.state_until) {
                    switch (bank.state) {
                        case hbm3::BankState::ACTIVATING:
                            bank.state = hbm3::BankState::ACTIVE;
                            trace_bank_state_change(ch, pc, b, hbm3::BankState::ACTIVE, "tRCD complete");
                            break;
                        case hbm3::BankState::PRECHARGING:
                            bank.state = hbm3::BankState::IDLE;
                            bank.open_row = 0;
                            bank.last_read_cmd = 0;
                            bank.last_write_cmd = 0;
                            trace_bank_state_change(ch, pc, b, hbm3::BankState::IDLE, "tRP complete");
                            break;
                        case hbm3::BankState::REFRESHING:
                            bank.state = hbm3::BankState::IDLE;
                            trace_bank_state_change(ch, pc, b, hbm3::BankState::IDLE, "tRFCpb complete");
                            break;
                        default:
                            break;
                    }
                ...
                if (current_cycle_ >= bank.burst_end) {
                    if (bank.state == hbm3::BankState::READING || bank.state == hbm3::BankState::WRITING) {
                        bank.state = hbm3::BankState::ACTIVE;
                        trace_bank_state_change(ch, pc, b, hbm3::BankState::ACTIVE, "burst complete");
                    }
                }
            }
        }
...
void HBM3MemoryController::update_bus_states() {
    for (uint8_t ch = 0; ch < hbm3_config_.num_channels; ++ch) {
        for (uint8_t pc = 0; pc < hbm3_config_.pseudo_channels_per_channel; ++pc) {
...
void HBM3MemoryController::finalize_bus_traces() {
    for (uint8_t ch = 0; ch < hbm3_config_.num_channels; ++ch) {
        for (uint8_t pc = 0; pc < hbm3_config_.pseudo_channels_per_channel; ++pc) {
            PseudoChannel& pseudo_ch = channels_[ch].pseudo_channels[pc];
...
void HBM3MemoryController::schedule_requests() {
...
bool HBM3MemoryController::try_issue_request(MemoryRequest& req) {
    ...
    switch (b.state) {
        ...
        case hbm3::BankState::ACTIVE:
        case hbm3::BankState::READING:
        case hbm3::BankState::WRITING:
            if (b.open_row == row) {
...
void HBM3MemoryController::complete_requests() {
...
void HBM3MemoryController::check_all_invariants() {
    for (uint8_t ch = 0; ch < hbm3_config_.num_channels; ++ch) {
        for (uint8_t pc = 0; pc < hbm3_config_.pseudo_channels_per_channel; ++pc) {
...
void HBM3MemoryController::check_bank_invariants(uint8_t channel, uint8_t pc, uint8_t bank) {
    ...
    if (b.state == hbm3::BankState::PRECHARGING && b.last_write_cmd > 0) {
        uint64_t write_done = b.last_write_cmd + timing.tWL + burst_cycles();
        if (current_cycle_ < write_done + timing.tWR) {
            report_violation("HBM3-INV-003",
    ...
    if (b.state == hbm3::BankState::PRECHARGING && b.last_read_cmd > 0) {
        if (current_cycle_ < b.last_read_cmd + timing.tRTP) {
            report_violation("HBM3-INV-004",
...
void HBM3MemoryController::check_timing_invariants(uint8_t channel, uint8_t pc) {
    ...
    if (count > 4) {
        report_violation("HBM3-INV-100",
            "tFAW violation: " + std::to_string(count) + " activates in window",
            channel, pc, 0);
    }
}

void HBM3MemoryController::check_bus_invariants(uint8_t channel, uint8_t pc) {
    ...
    if (pseudo_ch.data_bus_state != DataBusState::IDLE &&
        ...
        report_violation("HBM3-INV-201",
            "Data bus stuck in non-IDLE state past deadline",
            channel, pc, 0);
    }
}

void HBM3MemoryController::report_violation(const std::string& id, const std::string& msg,
...
void HBM3MemoryController::trace_bank_state_change(uint8_t channel, uint8_t pc, uint8_t bank,
...
void HBM3MemoryController::trace_bus_state_change(uint8_t channel, uint8_t pc, bool is_data_bus,
...
void HBM3MemoryController::trace_command(uint8_t channel, uint8_t pc, uint8_t bank,
