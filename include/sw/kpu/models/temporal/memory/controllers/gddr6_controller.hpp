...
// GDDR6 Memory Controller with formal invariant checking
//
// Implements cycle-accurate GDDR6 memory controller
...
struct TimingParams {
    ...
    uint32_t tRCDRD = 18;    // Row address to column address delay (read)
    uint32_t tRCDWR = 18;    // Row address to column address delay (write)
    uint32_t tRP = 18;       // Row precharge time
    uint32_t tRAS = 28;      // Row active time (minimum)
    uint32_t tRC = 46;       // Row cycle time (tRAS + tRP)
    uint32_t tRL = 20;       // CAS read latency
    ...
    // Burst timing (BL16, 4 WCK cycles)
...
enum class BurstLength : uint8_t {
    BL16 = 16  // GDDR6 always uses BL16
};
...
// ============================================================================
// Channel State (GDDR6 has two independent channels)
// ============================================================================
...
/// Cycle-accurate GDDR6 memory controller implementing IMemoryController
///
/// Provides full GDDR6 protocol simulation including:
/// - Bank state machine (IDLE, ACTIVATING, ACTIVE, READING, WRITING, PRECHARGING, REFRESHING)
/// - Dual-channel operation (two independent 16-bit channels)
...
class GDDR6MemoryController : public sw::kpu::IMemoryController {
public:
    /// GDDR6-specific configuration
    struct Config {
        uint8_t num_channels = 2;        // Always 2 for GDDR6
        uint8_t banks_per_channel = 16;  // Always 16
    ...
    explicit GDDR6MemoryController(const Config& config);

    /// Construct from generic MemoryControllerConfig
    explicit GDDR6MemoryController(const sw::kpu::MemoryControllerConfig& config);

    ~GDDR6MemoryController() override = default;
    ...
    sw::kpu::MemoryTechnology technology() const override {
        return sw::kpu::MemoryTechnology::GDDR6;
    }
    ...
    uint8_t num_channels() const override { return gddr6_config_.num_channels; }
    uint8_t banks_per_channel() const override { return gddr6_config_.banks_per_channel; }
    ...
    // ========================================================================
    // GDDR6-Specific API (not part of IMemoryController)
    // ========================================================================

    /// Get GDDR6-specific bank state (more detailed than interface)
    gddr6::BankState get_gddr6_bank_state(uint8_t channel, uint8_t bank) const;

    /// Check if a bank is idle
    bool is_bank_idle(uint8_t channel, uint8_t bank) const;

    /// Get GDDR6-specific statistics
    const Statistics& gddr6_stats() const { return stats_; }

    /// Get GDDR6-specific violations
    const std::vector<gddr6::InvariantViolation>& gddr6_violations() const { return violations_; }
    ...
    /// Get GDDR6-specific configuration
    const Config& gddr6_config() const { return gddr6_config_; }

    /// Get timing parameters
    const TimingParams& timing() const { return gddr6_config_.timing; }
...
private:
    ...
    // Tracing helpers
    void trace_bank_state_change(uint8_t channel, uint8_t bank, gddr6::BankState new_state,
    ...
    // Configuration
    Config gddr6_config_;                              // GDDR6-specific config
    ...
    std::vector<gddr6::InvariantViolation> violations_;
