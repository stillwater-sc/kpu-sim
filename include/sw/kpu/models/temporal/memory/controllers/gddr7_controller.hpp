...
// GDDR7 Memory Controller with formal invariant checking
//
// Implements cycle-accurate GDDR7 memory controller
...
// ============================================================================
// Timing Parameters (GDDR7-32000 @ 2.0 GHz CK, PAM3 signaling)
// ============================================================================

struct TimingParams {
    // Core timing (in CK cycles)
    // GDDR7 uses PAM3 signaling for ~1.5x effective data rate vs GDDR6
    uint32_t tRCDRD = 14;    // Row address to column address delay (read)
    uint32_t tRCDWR = 14;    // Row address to column address delay (write)
    uint32_t tRP = 14;       // Row precharge time
    uint32_t tRAS = 24;      // Row active time (minimum)
    uint32_t tRC = 38;       // Row cycle time (tRAS + tRP)
    uint32_t tRL = 16;       // CAS read latency
    uint32_t tWL = 6;        // CAS write latency
    uint32_t tWR = 14;       // Write recovery time
    ...
    uint32_t tCCD_L = 3;     // CAS to CAS (same bank group)
    uint32_t tCCD_S = 2;     // CAS to CAS (different bank group)

    // Turnaround timing
    uint32_t tWTR_L = 5;     // Write to read (same bank group)
    uint32_t tWTR_S = 3;     // Write to read (different bank group)
    uint32_t tRTW = 12;      // Read to write (bus turnaround)

    // Burst timing (BL16, 4 WCK cycles with PAM3)
    uint32_t tBurst = 4;     // Burst length 16 cycles (in CK)

    // Refresh timing (per-bank)
    uint32_t tRFCpb = 110;   // Per-bank refresh cycle time
    uint32_t tRFCab = 220;   // All-bank refresh cycle time
    uint32_t tREFI = 3900;   // Refresh interval

    // Activate window
    uint32_t tFAW = 12;      // Four activate window
};
...
enum class BurstLength : uint8_t {
    BL16 = 16,   // Standard burst
    BL32 = 32    // Extended burst for high throughput
};
...
// ============================================================================
// Channel State (GDDR7 has two independent channels)
// ============================================================================
...
struct Channel {
    ...
    // Banks and bank groups (16 banks: 4 bank groups × 4 banks)
...
/// Cycle-accurate GDDR7 memory controller implementing IMemoryController
///
/// Provides full GDDR7 protocol simulation including:
/// - Bank state machine (IDLE, ACTIVATING, ACTIVE, READING, WRITING, PRECHARGING, REFRESHING)
/// - Dual-channel operation (two independent 16-bit channels with PAM3)
/// - Per-bank refresh scheduling
/// - Bank group timing constraints (tRRD_L, tCCD_L, tWTR_L)
/// - tFAW (four-activate window) tracking
/// - PAM3 signaling for ~32 Gbps effective data rate
/// - Formal invariant checking
/// - Full tracing support for visualization
class GDDR7MemoryController : public sw::kpu::IMemoryController {
public:
    /// GDDR7-specific configuration
    struct Config {
        uint8_t num_channels = 2;        // Always 2 for GDDR7
        uint8_t banks_per_channel = 16;  // 4 bank groups × 4 banks
    ...
    explicit GDDR7MemoryController(const Config& config);

    /// Construct from generic MemoryControllerConfig
    explicit GDDR7MemoryController(const sw::kpu::MemoryControllerConfig& config);

    ~GDDR7MemoryController() override = default;
    ...
    sw::kpu::MemoryTechnology technology() const override {
        return sw::kpu::MemoryTechnology::GDDR7;
    }
    ...
    uint8_t num_channels() const override { return gddr7_config_.num_channels; }
    uint8_t banks_per_channel() const override { return gddr7_config_.banks_per_channel; }
    ...
    // ========================================================================
    // GDDR7-Specific API (not part of IMemoryController)
    // ========================================================================

    /// Get GDDR7-specific bank state (more detailed than interface)
    gddr7::BankState get_gddr7_bank_state(uint8_t channel, uint8_t bank) const;

    /// Check if a bank is idle
    bool is_bank_idle(uint8_t channel, uint8_t bank) const;

    /// Get GDDR7-specific statistics
    const Statistics& gddr7_stats() const { return stats_; }

    /// Get GDDR7-specific violations
    const std::vector<gddr7::InvariantViolation>& gddr7_violations() const { return violations_; }
    ...
    /// Get GDDR7-specific configuration
    const Config& gddr7_config() const { return gddr7_config_; }

    /// Get timing parameters
    const TimingParams& timing() const { return gddr7_config_.timing; }
...
private:
    ...
    // Tracing helpers
    void trace_bank_state_change(uint8_t channel, uint8_t bank, gddr7::BankState new_state,
    ...
    // Configuration
    Config gddr7_config_;                              // GDDR7-specific config
    ...
    std::vector<gddr7::InvariantViolation> violations_;
