...
// HBM2 Memory Controller with formal invariant checking
//
// Implements cycle-accurate HBM2 memory controller
...
struct TimingParams {
    // Core timing (in CK cycles @ 1.0 GHz)
    uint32_t tRCDRD = 12;    // Row address to column address delay (read)
    uint32_t tRCDWR = 6;     // Row address to column address delay (write)
    uint32_t tRP = 14;       // Row precharge time
    uint32_t tRAS = 28;      // Row active time (minimum)
    uint32_t tRC = 42;       // Row cycle time (tRAS + tRP)
    uint32_t tRL = 18;       // CAS read latency
    uint32_t tWL = 7;        // CAS write latency
    uint32_t tWR = 16;       // Write recovery time
    ...
    // Turnaround timing
    uint32_t tWTR_L = 8;     // Write to read (same bank group)
    uint32_t tWTR_S = 4;     // Write to read (different bank group)
    uint32_t tRTW = 10;      // Read to write (bus turnaround)

    // Burst timing (BL4 pseudo-channel mode)
    uint32_t tBurst = 2;     // Burst length 4 (in CK)
...
enum class BurstLength : uint8_t {
    BL4 = 4    // HBM2 uses BL4 in pseudo-channel mode
};
...
// ============================================================================
// Pseudo-Channel State (HBM2 has 2 pseudo-channels per channel)
// ============================================================================
...
struct PseudoChannel {
    ...
    // Data bus (64-bit per pseudo-channel)
...
// ============================================================================
// Channel State (HBM2 has 8 channels per stack)
// ============================================================================
...
struct Statistics {
    ...
    // Per-channel stats (8 channels)
    std::array<uint64_t, 8> channel_accesses = {0};

    // Per-pseudo-channel stats (16 total)
    std::array<uint64_t, 16> pseudo_channel_accesses = {0};
...
/// Cycle-accurate HBM2 memory controller implementing IMemoryController
///
/// Provides full HBM2 protocol simulation including:
/// - Bank state machine (IDLE, ACTIVATING, ACTIVE, READING, WRITING, PRECHARGING, REFRESHING)
/// - 8 channels per stack with 2 pseudo-channels each (16 pseudo-channels total)
/// - 16 banks per pseudo-channel (256 banks total per stack)
...
class HBM2MemoryController : public sw::kpu::IMemoryController {
public:
    /// HBM2-specific configuration
    struct Config {
        ...
        BurstLength burst_length = BurstLength::BL4;
    ...
    explicit HBM2MemoryController(const Config& config);

    /// Construct from generic MemoryControllerConfig
    explicit HBM2MemoryController(const sw::kpu::MemoryControllerConfig& config);

    ~HBM2MemoryController() override = default;
    ...
    sw::kpu::MemoryTechnology technology() const override {
        return sw::kpu::MemoryTechnology::HBM2;
    }
    ...
    uint8_t num_channels() const override { return hbm2_config_.num_channels; }
    uint8_t banks_per_channel() const override {
        return hbm2_config_.pseudo_channels_per_channel * hbm2_config_.banks_per_pseudo_channel;
    }
    ...
    // ========================================================================
    // HBM2-Specific API (not part of IMemoryController)
    // ========================================================================

    /// Get HBM2-specific bank state (more detailed than interface)
    hbm2::BankState get_hbm2_bank_state(uint8_t channel, uint8_t pc, uint8_t bank) const;

    /// Check if a bank is idle
    bool is_bank_idle(uint8_t channel, uint8_t pc, uint8_t bank) const;

    /// Get HBM2-specific statistics
    const Statistics& hbm2_stats() const { return stats_; }

    /// Get HBM2-specific violations
    const std::vector<hbm2::InvariantViolation>& hbm2_violations() const { return violations_; }
    ...
    /// Get HBM2-specific configuration
    const Config& hbm2_config() const { return hbm2_config_; }

    /// Get timing parameters
    const TimingParams& timing() const { return hbm2_config_.timing; }
...
private:
    ...
    // Configuration
    Config hbm2_config_;                              // HBM2-specific config
    ...
    std::vector<hbm2::InvariantViolation> violations_;
