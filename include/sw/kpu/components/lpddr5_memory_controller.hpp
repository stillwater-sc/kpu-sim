...
// LPDDR5 Memory Controller with formal invariant checking
//
// Implements the state model from docs/LPDDR5_STATE_MODEL.md
...
// ============================================================================
// Timing Parameters (LPDDR5-6400 @ 3200 MHz)
// ============================================================================

struct TimingParams {
    // Core timing
    uint32_t tRCD = 14;      // Row address to column address delay
    ...
    // Burst timing
    uint32_t tBurst_BL16 = 8;   // Burst length 16 cycles
    uint32_t tBurst_BL32 = 16;  // Burst length 32 cycles

    // Refresh timing (per-bank)
    uint32_t tRFCpb = 140;   // Per-bank refresh cycle time
    uint32_t tRFCab = 280;   // All-bank refresh cycle time
    uint32_t tREFIpb = 244;  // Per-bank refresh interval
...
/// Cycle-accurate LPDDR5 memory controller implementing IMemoryController
///
/// Provides full LPDDR5 protocol simulation including:
...
class LPDDR5MemoryController : public sw::kpu::IMemoryController {
public:
    /// LPDDR5-specific configuration
    struct Config {
        uint8_t num_channels = 1;        // 1 or 2
    ...
    explicit LPDDR5MemoryController(const Config& config);

    /// Construct from generic MemoryControllerConfig
    explicit LPDDR5MemoryController(const sw::kpu::MemoryControllerConfig& config);

    ~LPDDR5MemoryController() override = default;
    ...
    sw::kpu::MemoryTechnology technology() const override {
        return sw::kpu::MemoryTechnology::LPDDR5;
    }
    ...
    uint8_t num_channels() const override { return lpddr5_config_.num_channels; }
    uint8_t banks_per_channel() const override { return lpddr5_config_.banks_per_channel; }
    ...
    /// Get LPDDR5-specific bank state (more detailed than interface)
    lpddr5::BankState get_lpddr5_bank_state(uint8_t channel, uint8_t bank) const;

    /// Check if a bank is idle
    bool is_bank_idle(uint8_t channel, uint8_t bank) const;

    /// Get LPDDR5-specific statistics
    const Statistics& lpddr5_stats() const { return stats_; }

    /// Get LPDDR5-specific violations
    const std::vector<lpddr5::InvariantViolation>& lpddr5_violations() const { return violations_; }
    ...
    /// Get LPDDR5-specific configuration
    const Config& lpddr5_config() const { return lpddr5_config_; }

    /// Get timing parameters
    const TimingParams& timing() const { return lpddr5_config_.timing; }
