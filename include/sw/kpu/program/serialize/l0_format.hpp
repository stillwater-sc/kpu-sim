...
inline void write_l0(std::ostream& os, const TileProgram& prog) {
    ...
    for (const TileOp& op : prog.ops()) {
        ...
        // alpha is written only when it is not the default, so a diff of two
        // programs shows what actually differs rather than what is merely present.
...
inline Dim to_dim(const std::string& s, const std::string& where) {
    try {
        ...
        throw FormatError(FormatError::Cause::MalformedRecord,
                          "l0: " + where + ": '" + s + "' is not a non-negative integer");
    }
}
...
inline TileProgram read_l0(std::istream& is) {
    ...
    while (std::getline(is, line)) {
        ...
        if (kw == "MIN_CONSUMER") {
            ...
            if (f.count("alpha")) {
                try {
                    std::size_t used = 0;
                    op.alpha = std::stof(f["alpha"], &used);
                    if (used != f["alpha"].size()) throw std::invalid_argument("trailing");
                } catch (const std::exception&) {
                    throw FormatError(FormatError::Cause::MalformedRecord,
                                      "l0: OP: alpha '" + f["alpha"] + "' is not a number");
                }
            }
            if (f.count("pivot"))
                op.pivot_slot = static_cast<int>(detail::to_dim(f["pivot"], "OP pivot"));
    ...
    for (TensorOperand& o : operands) out.add_operand(std::move(o));
