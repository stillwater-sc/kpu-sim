...
public:
    ...
    Schedule list_schedule() {
        ...
        for (std::size_t i = 0; i < n; ++i) height_(i, height);
...
private:
    ...
    void build_(const TileProgram& prog) {
        ...
        for (std::size_t i = 0; i < ops.size(); ++i) {
            ...
            switch (op.kind) {
                case TileOpKind::Feed:          reads.push_back(&op.inputs[0]); break;
                case TileOpKind::Drain:         reads.push_back(&op.outputs[0]); break;
                case TileOpKind::MatMulAccum:
            ...
            for (const TileCoord* t : reads) {
                ...
                if (it != last_writer.end()) add_dep(i, it->second);     // RAW
