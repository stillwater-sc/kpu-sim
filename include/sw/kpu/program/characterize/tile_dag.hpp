...
// The structural work model lives in the portable-program layer so the executor
// and this harness cost an op identically (tile_work.hpp).
using TileWork = program::TileWork;
...
private:
    ...
    void build_(const TileProgram& prog) {
        ...
        for (std::size_t i = 0; i < ops.size(); ++i) {
            ...
            nodes_[i].work = tile_work_of(prog, op, dev_.element_bytes);
            nodes_[i].duration = lumped_duration(nodes_[i].work,
                                                dev_.fabric_macs_per_cycle,
                                                dev_.bytes_per_cycle);

            // L1-timed override: systolic wavefront for compute, bubble-scaled drain.
            if (l1_) nodes_[i].duration = l1_duration(prog, op, i, nodes_[i].duration, *l1_);
